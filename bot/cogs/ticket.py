...
class Ticket(commands.Cog):
    ...
    async def ticket(self, ctx):
        ...
        for channel in ticket_category.text_channels:
            ...
            if tickets > 2:
                message_embed = create_embed('Open tickets', 'Please refer to one of these')

                for ticket in tickets_array:
                    message_embed.add_field(name='Open ticket', value=ticket.mention)
                message = await ctx.send(f'{ctx.message.author.mention}: You already have 3 open tickets, please refer to one of these', embed=message_embed)
